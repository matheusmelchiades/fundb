    ...
    pub async fn simple_query(&mut self, sql: &str) -> anyhow::Result<QueryResponse> {
        ...
        loop {
            ...
            match msg_type {
                ...
                b'C' => {
                    // CommandComplete
                    response.command_tag = parse_cstring(&payload, 0)
                        .unwrap_or_default()
                        .to_string();
                }
                b'E' => {
...
fn read_i32_be(buf: &[u8], offset: usize) -> anyhow::Result<i32> {
    if buf.len() < offset + 4 {
        return Err(anyhow::anyhow!("Buffer too short to read i32 at {}", offset));
    }
